...
class Wind(Profile):
    ...
    def __init__(self, l, scale_factor=1, min_shift=8, max_shift=8, total_shift=0, cost_min=0, cost_max=0, timestamps=96):
        super().__init__([x * scale_factor if x < 0 else x for x in l], ProfileType.WIND, cost_min, cost_max, timestamps)
        ...
        self.min_flex = self.setup_array_for_property([((x - (x * (min_shift / 100)* self.scale_factor))) for x in l])
        self.max_flex = self.setup_array_for_property([((x + (x * (max_shift / 100)* self.scale_factor))) for x in l])
