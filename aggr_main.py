...
def print_graph(plant, title=''):
    ...
    ax.set(xlabel='Flexibility', ylabel='Active Power (W)', title=title)
    plt.xticks(range(0, 96, 5))

    ax.plot(plant.profile, label='Baseline')
    ax.plot(plant.get_min_flexibility(), label='Min')
    ax.plot(plant.get_max_flexibility(), label='Max')
...
def plot_results(result, resolve_method='maximized', print_costs=True):
    ...
    ax.plot(result[resolve_method]['MAX_BOUND'], label='MAX_opt (' + resolve_method + ')')
    ax.plot(result[resolve_method]['MIN_BOUND'], label='MIN_opt (' + resolve_method + ')')

    ax.plot(result[resolve_method]['BASELINE'], label='baseline (' + resolve_method + ')')
    plt.legend(bbox_to_anchor=(1, 1), loc=1, borderaxespad=0.3)
    plt.show()
    ...
    if print_costs:
        ...
        # ax.plot(result[resolve_method]['GAIN'], label='OPT_GAIN (' + resolve_method + ')')
        ...
        ax.plot([x * y for x, y in zip(result[resolve_method]['GAIN_MAX'], result[resolve_method]['MAX_BOUND'])],
                label='GAIN_max (' + resolve_method + ')')
        ax.plot([x * y for x, y in zip(result[resolve_method]['GAIN_MIN'], result[resolve_method]['MIN_BOUND'])],
                label='GAIN_min (' + resolve_method + ')')
...
pontlab1_test = np.load('Baseline/' + date + '.uvax.27.plants.1041.profile.baseline.npy') / scale_factor
pontlab2_test = np.load('Baseline/' + date + '.uvax.27.plants.1042.profile.baseline.npy') / scale_factor
load4_test = np.load('Baseline/' + date + '.uvax.27.plants.1043.profile.baseline.npy') / scale_factor
pv1_test = np.load('Baseline/' + date + '.uvax.27.plants.1044.profile.baseline.npy') / scale_factor
pv2_test = np.load('Baseline/' + date + '.uvax.27.plants.1045.profile.baseline.npy') / 10
wind1_test = np.load('Baseline/' + date + '.uvax.27.plants.1046.profile.baseline.npy') * 10
wind2_test = np.load('Baseline/' + date + '.uvax.27.plants.1047.profile.baseline.npy') * 10
load1_test = np.load('Baseline/' + date + '.uvax.27.plants.1048.profile.baseline.npy') * 10000
load2_test = np.load('Baseline/' + date + '.uvax.27.plants.1049.profile.baseline.npy') * 100
load3_test = np.load('Baseline/' + date + '.uvax.27.plants.1050.profile.baseline.npy') / scale_factor
bess1_test = np.load('Baseline/' + date + '.uvax.27.plants.1051.profile.baseline.npy') / scale_factor
bess2_test = np.load('Baseline/' + date + '.uvax.27.plants.1052.profile.baseline.npy') / scale_factor
chp1_test = np.load('Baseline/' + date + '.uvax.27.plants.1041.profile.baseline.npy') / scale_factor
...
load3 = LoadT3(load3_test, [x for x in list(range(0, 96)) if x not in list(range(0, 20))],
               0)  # max(kW) = 12 [shift - no_loss]
load4 = LoadT3(load4_test, [x for x in list(range(0, 96)) if x not in list(range(55, 60))],
               10)  # max(kW) = 11 [shift - loss]
p_load1 = Pod(load1)
p_load2 = Pod(load2)
p_load3 = Pod(load3)
p_load4 = Pod(load4)
...
#  CONF_5: 2PV,2wind,1St,      1L1,3L2,2L3
...
p4.set_profiles([p4_pv_1, p4_wind_1, p4_st,
                 ...
                 p4_l3_1])
########################################################################################
# CONF_5    ->     [2 PV / 2 WIND / 1 ST / 1 L1 / 3 L2 / 2 L3 (l4)]
p5 = Pod()
...
p7_l2_1 = LoadT2(load1_test, list(range(0, 96)), 10)  # max(kW) = 0.0012 [no_shift - loss]
p7_l2_2 = LoadT2(load1_test, list(range(0, 96)), 10)  # max(kW) = 0.0012 [no_shift - loss]
